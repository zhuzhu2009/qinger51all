...
#define	Timer2_Reload	(MAIN_Fosc / 100)		//Timer 2 中断频率, 100次/秒
...
const unsigned short code music_little_star[] = { // 4/4拍 unit: 1/4
...
void timer0_int (void) interrupt TIMER0_VECTOR
{
	if (music_first)
	{
		...
		PWMx_SetPwmWide_int0(PWM7_ID, 0, music_little_star[music_pos]>>1);
		music_pos++;
	}
	else
	{
...
void Timer2_init(void)
{
	Timer2_Stop();
		...
		Timer2_CLKO_Disable();
			...
			Timer2_1T();	//1T mode
			T2_Load(65536UL - Timer2_Reload);
		#else
			Timer2_12T();	//12T mode
			T2_Load(65536UL - Timer2_Reload/12);
		#endif
...
void timer3_int (void) interrupt TIMER3_VECTOR
{
	...
	PWMx_SetPwmWide_int3(PWM6_ID, 0, rc_out);
	rc_out+=4;
	if (rc_out >= 40)
	{
		rc_out = 4;
